...
void ROSThread::ros_initialize(ros::NodeHandle &n) {
  ...
  stop_sub_ = n.subscribe<std_msgs::Bool>(
      "/file_player_stop", 1,
      boost::bind(&ROSThread::FilePlayerStop, this, _1));

  clock_pub_ = n.advertise<rosgraph_msgs::Clock>("/clock", 1);
  gps_pub_ = n.advertise<sensor_msgs::NavSatFix>("/gps/fix", 1000);
  imu_pub_ = n.advertise<sensor_msgs::Imu>("/imu/data_raw", 1000);
  magnet_pub_ = n.advertise<sensor_msgs::MagneticField>("/mag_data", 1000);
  ouster_pub_ =
      n.advertise<sensor_msgs::PointCloud2>("/os1_points", 1000);  // giseop
  radarpolar_pub_ =
      n.advertise<sensor_msgs::Image>("/radar/polar", 10);  // giseop
}
