...
void ROSThread::ros_initialize(ros::NodeHandle &n) {
  ...
  ouster_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("/os1_points", 1000);  // giseop
  radarpolar_pub_ =
...
void ROSThread::Ready() {
  ...
  if (!f.good()) {
    cout << "Please check the file path. The input path is wrong "
            "(data_stamp.csv not exist)"
         << endl;
    return;
  }
  f.close();
  ...
  if (imu_active_) {
    io::CSVReader<17> in(
        (data_folder_path_ + "/sensor_data/xsens_imu.csv").c_str());
  ...
  GetDirList(data_folder_path_ + "/sensor_data/radar/polar",
             radarpolar_file_list_);
    ...
    while (!imu_thread_.data_queue_.empty()) {
      ...
      if (imu_data_.find(data) != imu_data_.end()) {
        ...
        if (imu_data_version_ == 2) {
          if (magnet_pub_.getNumSubscribers() != 0)
            magnet_pub_.publish(
                mag_data_[data]);  // Warning publisher has not been initialized
        }
      }
    }
    if (imu_thread_.active_ == false) return;
  ...
  while (1) {
    ...
    while (!ouster_thread_.data_queue_.empty()) {
      ...
      if (to_string(data) + ".bin" == ouster_next_.first) {
        ...
        string current_file_name = data_folder_path_ + "/sensor_data/Ouster" +
                                   "/" + to_string(data) + ".bin";

        if (find(next(ouster_file_list_.begin(),
                      max(0, previous_file_index - search_bound_)),
                 ouster_file_list_.end(),
                 to_string(data) + ".bin") != ouster_file_list_.end()) {
          ifstream file;
      ...
      sensor_msgs::PointCloud2 publish_cloud;
      current_file_index =
          find(next(ouster_file_list_.begin(),
                    max(0, previous_file_index - search_bound_)),
               ouster_file_list_.end(), to_string(data) + ".bin") -
          ouster_file_list_.begin();
      if (find(next(ouster_file_list_.begin(),
                    max(0, previous_file_index - search_bound_)),
               ouster_file_list_.end(),
               ouster_file_list_[current_file_index + 1]) !=
          ouster_file_list_.end()) {
        string next_file_name = data_folder_path_ + "/sensor_data/Ouster" +
                                "/" + ouster_file_list_[current_file_index + 1];
        ...
        pcl::toROSMsg(cloud, publish_cloud);
        ouster_next_ =
            make_pair(ouster_file_list_[current_file_index + 1], publish_cloud);
      }
  ...
  while (1) {
    ...
    while (!radarpolar_thread_.data_queue_.empty()) {
      ...
      if (to_string(data) + ".png" == radarpolar_next_.first &&
        ...
        string current_radarpolar_name = data_folder_path_ +
                                         "/sensor_data/radar/polar" + "/" +
                                         to_string(data) + ".png";

        cv::Mat radarpolar_image;
        radarpolar_image =
            imread(current_radarpolar_name, CV_LOAD_IMAGE_GRAYSCALE);
...
void ROSThread::SaveRosbag() {
  ...
  for (auto &&file_name : radarpolar_file_list_) {
    ...
    const std::string file_path =
        data_folder_path_ + "/sensor_data/radar/polar/" + file_name;
    cout << "radar: " << count++ << "/" << radarpolar_file_list_.size() << endl;
    // cout<<"load ("<<count++<<"/"<<radarpolar_file_list_.size()<<") from:
    // "<<file_path<<endl;
  ...
  if (fin.is_open()) {
    ...
    while (fin >> temp) {
      ...
      // cout<<"Written: "<<count++<<" /gt nav_msgs/odometry poses to
      // /gt"<<endl;
    }
  }
